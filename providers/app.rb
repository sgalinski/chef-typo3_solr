action :add do
  ...
  directory "#{node[:typo3_solr][:solr][:solr_home]}/#{new_resource.name}/typo3cores" do
    owner node["tomcat"]["user"]
    group "root"
    mode 0664
    action :create
  ...
  if Gem::Version.new(new_resource.extension) >= Gem::Version.new('2.8.0') then
  ...
  template "#{node[:tomcat][:context_dir]}/#{new_resource.name}.xml" do
    ...
    variables(:version => new_resource.solr)
    owner node["tomcat"]["user"]
    mode 0644
  end
  ...
  execute "chown" do
    command "chown -R #{node["tomcat"]["user"]}:#{node["tomcat"]["group"]} #{node[:typo3_solr][:solr][:solr_home]}/#{new_resource.name} && chmod -R a+x #{node[:typo3_solr][:solr][:solr_home]}/#{new_resource.name}"
